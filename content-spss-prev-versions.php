
  <div id="<?php echo $blocks[$counter]; ?>" class="row mainContent inline-content-imgright" data-block="<?php echo $blocks[$counter]; ?>">
      <div class="container">
          <div class="row">
               <div class="large-5 columns grid-icons">
				   <a href="https://estore.onthehub.com/WebStore/OfferingsOfMajorVersionList.aspx?pmv=fc2dece7-5e83-e711-80f3-000d3af41938&utm_source=spss-statistics-25-img&utm_medium=othlandingpage&utm_campaign=SPSS" target="_blank" class='main-prev'><img src="img/spss-25-boxshot.jpg" alt="SPSS Statistics 25 GradPack" /></a>
				   <div class="sub-prev">
					   <a href="https://estore.onthehub.com/WebStore/OfferingsOfMajorVersionList.aspx?pmv=12c7bd0a-436e-e511-9411-b8ca3a5db7a1&utm_source=spss-statistics-24-img&utm_medium=othlandingpage&utm_campaign=SPSS" target="_blank"><img src="img/spss-24-boxshot.jpg" alt="SPSS Statistics 24"></a>
					   <a href="https://estore.onthehub.com/WebStore/OfferingsOfMajorVersionList.aspx?pmv=89cf975c-47c3-e411-940a-b8ca3a5db7a1&utm_source=spss-statistics-23-img&utm_medium=othlandingpage&utm_campaign=SPSS" target="_blank"><img src="img/spss-23-boxshot.jpg" alt="SPSS Statistics 23"></a>
				   </div>
              </div>
              <div class="large-6 columns info">
                  <div class="row title"> 
                        <h2>Looking for previous versions?</h2>
                  </div> 
                  	<p>You can still order <strong>IBM<sup style="font-size:0.5em;"><sup>&reg;</sup></sup> SPSS<sup style="font-size:0.5em;"><sup>&reg;</sup></sup> Statistics 23, 24,</strong> and <strong>25</strong> through OnTheHub. Students and faculty can both take comfort in knowing that the software they use matches course materials.</p>
				  	<p><strong class="red">$34.95 USD</strong> – Students <strong>Save 96%</strong></p>
                  <br />
                  <div class="row ctaBtns">
                      <a href="https://estore.onthehub.com/WebStore/ProductsByMajorVersionList.aspx?cmi_mnuMain=8659da1b-241d-e611-941a-b8ca3a5db7a1&pc=ddc848d8-b4fe-e111-bd05-f04da23e67f6&utm_source=spss-all-previousversions-cta&utm_medium=othlandingpage&utm_medium=othlandingpage&utm_campaign=SPSS" target="_blank" onclick="tob(this, '<?php echo $gaEvent; ?>', '<?php echo $gaEvent; ?> - Clickthrough', '<?php echo $gaEvent; ?> v2 - Clickthrough - Previous Versions - Buy'); return false;">
                          <div class="large-6 columns button rent" target="_blank">
                             <span>Buy now</span>
                          </div>
                      </a>    
                  </div>
              </div>
          </div> 
      </div>  
  </div>